...
type SessionData  = { trades: number; wins: number; pnl: number; win_rate: number };
...
// ── helpers ───────────────────────────────────────────────────────────────────
function f2(n: number) { return isFinite(n) ? n.toFixed(2) : "-"; }
function pct(n: number) { return `${n >= 0 ? "+" : ""}${f2(n)}%`; }
function dollar(n: number) { return `${n >= 0 ? "+" : "-"}$${Math.abs(n).toFixed(2)}`; }
function clr(n: number) { return n >= 0 ? GN : RD; }

function card(style?: React.CSSProperties): React.CSSProperties {
  return { background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14,
...
function EquityCurve({ data }: { data: Array<{ equity: number }> }) {
  ...
  const fillD = `M${tx(0).toFixed(1)},${(H - P).toFixed(1)} L${pts} L${tx(vals.length - 1).toFixed(1)},${(H - P).toFixed(1)} Z`;
  const rising = vals[vals.length - 1] >= vals[0];
  const color = rising ? "#00ffd1" : "#ff5078";
  return (
...
function DrawdownCurve({ data }: { data: Array<{ dd: number }> }) {
  ...
  const min = Math.min(...vals); // most negative
  if (min >= 0) {
    return <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: GN, fontSize: 12 }}>No drawdown — perfect run</div>;
  }
  const ty = (v: number) => P + (v / min) * (H - P * 2);
  const tx = (i: number) => P + (i / (vals.length - 1)) * (W - P * 2);
  const pts = vals.map((v, i) => `${tx(i).toFixed(1)},${ty(v).toFixed(1)}`).join(" ");
  const fillD = `M${tx(0).toFixed(1)},${P} L${pts} L${tx(vals.length - 1).toFixed(1)},${P} Z`;
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "100%" }} preserveAspectRatio="none">
      <defs>
        <linearGradient id="dd-g" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#ff5078" stopOpacity=".04" />
          <stop offset="100%" stopColor="#ff5078" stopOpacity=".38" />
        </linearGradient>
      </defs>
      <line x1={P} y1={P} x2={W - P} y2={P} stroke="rgba(255,255,255,.12)" strokeWidth="1" />
      <path d={fillD} fill="url(#dd-g)" />
...
// ── trade card (best / worst) ─────────────────────────────────────────────────
function TradeCard({ trade, label }: { trade: TradeRec; label: string }) {
  const pnlVal = trade.unreal_pnl_value;
  const pnlPct = trade.unreal_pnl_percent;
  const isWin  = pnlVal >= 0;
  const accent = isWin ? GN : RD;
  const lines  = (trade.reason || "").split("\n").filter(Boolean);
  ...
  return (
    <div style={{ ...card(), padding: "14px 16px" }}>
      ...
      <div style={{ fontSize: 22, fontWeight: 900, color: accent }}>{dollar(pnlVal)}</div>
      <div style={{ fontSize: 13, color: accent, marginBottom: 10 }}>{pct(pnlPct)} leveraged</div>
      <div style={{ fontSize: 11, color: M, marginBottom: 4 }}>{trade.symbol} · {trade.side} · {trade.mode}</div>
      {signalLine  && <div style={{ fontSize: 11, color: S, marginBottom: 2 }}>{signalLine.trim()}</div>}
      {outcomeLine && <div style={{ fontSize: 11, color: S, marginBottom: 2 }}>{outcomeLine.trim()}</div>}
      {styleLine   && <div style={{ fontSize: 10, color: "rgba(255,255,255,.25)", marginTop: 6 }}>{styleLine.trim()}</div>}
    </div>
  );
}

// ── mini bar (for breakdown tables) ──────────────────────────────────────────
...
export default function Portfolio() {
  ...
  const load = async () => {
    ...
    try {
      const data = await api.apiRequest<Stats>("/portfolio/stats", { method: "GET" });
      setStats(data);
    } catch (e: any) {
  ...
  const totalWins   = summary.total_trades * (summary.win_rate / 100);
  ...
  const summaryCards = [
    ...
    { label: "Win Rate",       value: `${summary.win_rate}%`,              sub: `${Math.round(totalWins)}W / ${Math.round(totalLosses)}L`, color: summary.win_rate >= 50 ? GN : RD },
    ...
    { label: "Streak",         value: `${summary.current_streak.count}×${summary.current_streak.type}`, sub: summary.current_streak.type === "W" ? "consecutive wins" : "consecutive losses", color: summary.current_streak.type === "W" ? GN : RD },
    { label: "Max Drawdown",   value: `-${f2(summary.max_drawdown)}%`,    sub: "from peak equity", color: summary.max_drawdown < 5 ? GN : summary.max_drawdown < 10 ? "rgba(251,191,36,.9)" : RD },
    { label: "Health Score",   value: String(summary.health_score),        sub: summary.health_score >= 70 ? "Strong" : summary.health_score >= 45 ? "Fair" : "Weak", color: summary.health_score >= 70 ? GN : summary.health_score >= 45 ? "rgba(251,191,36,.9)" : RD },
  ];
  ...
  return (
    <div style={pg}>
      <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", flexDirection: "column", gap: gridGap }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: gridGap }}>
          {summaryCards.map(c => (
            <div key={c.label} style={{ ...card(), padding: "12px 14px" }}>
              ...
              <div style={{ fontSize: 10, color: S, marginTop: 4 }}>{c.sub}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: gridGap }}>
          <div style={card()}>
            {head("Equity Curve — Full History")}
            <div style={{ padding: "10px 14px 14px", height: 150 }}>
          ...
          <div style={card()}>
            ...
            <div style={{ padding: "0 14px 12px", fontSize: 10, color: RD }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 220px", gap: gridGap }}>
          <div style={card()}>
            {head("Monthly P&L")}
            <div style={{ padding: "10px 14px 14px", height: 120 }}>
              <MonthlyBars data={monthly_pnl} />
            </div>
          ...
          <div style={card()}>
            {head("Win / Loss")}
            <div style={{ padding: 14, height: 130 }}>
              <WinRing wins={Math.round(totalWins)} losses={Math.round(totalLosses)} rate={summary.win_rate} />
            </div>
          </div>
        </div>

        {/* ── consistency + mistakes + grades + ratios ── */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: gridGap }}>

          {/* consistency */}
          <div style={{ ...card(), padding: "14px 16px", display: "flex", flexDirection: "column", gap: 18 }}>
            <ScoreBar score={summary.consistency_score} label="Trade Consistency" color={summary.consistency_score >= 65 ? GN : "rgba(251,191,36,.9)"} />
            <ScoreBar score={summary.health_score}      label="Health Score"      color={summary.health_score >= 65 ? GN : "rgba(251,191,36,.9)"}      />
          </div>

          {/* mistake detection */}
          <div style={card()}>
            {head("Mistake Detection")}
          ...
          <div style={card()}>
            ...
            <div style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <div style={{ marginTop: 4, fontSize: 10, color: S }}>
                Grade A = full size · Grade B = T1+T2 split
              </div>
            </div>
          </div>
          ...
          <div style={{ ...card(), padding: "14px 16px" }}>
            ...
            {[
              ...
              { k: "Sortino", v: ratios.sortino, tip: "Mean / Downside dev" },
              { k: "Calmar",  v: ratios.calmar,  tip: "Annual return / Max DD" },
            ].map(r => (
            ...
            {summary.total_trades < 10 && <div style={{ fontSize: 10, color: S }}>Need more trades for reliable ratios</div>}
          </div>
        </div>

        {/* ── session performance ── */}
        <div style={card()}>
          {head("Session Performance")}
          <div style={{ padding: "10px 14px 14px", overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 520 }}>
              <thead>
                <tr>
                  {["Session", "Trades", "Win Rate", "P&L", "Win/Loss Bar"].map(h => (
                ...
                {Object.entries(session_distribution).sort((a, b) => b[1].trades - a[1].trades).map(([sess, d]) => (
                  <tr key={sess}>
                    ...
                    <td style={{ padding: "8px 10px", minWidth: 120 }}>
                      <MiniBar pct={d.win_rate} color={d.win_rate >= 50 ? GN : RD} />
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        {/* ── time of day heatmap ── */}
        <div style={card()}>
          {head("Time of Day Heatmap (Dubai Time)")}
          <div style={{ padding: "12px 14px 16px" }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(24,1fr)", gap: 3, marginBottom: 8 }}>
              {Array.from({ length: 24 }, (_, h) => {
                ...
                const sessLabel = h < 2 ? "NY-c" : h < 6 ? h < 2 ? "NY-c" : "Dead" : h < 12 ? "Asia" : h < 16 ? "Lon" : h < 21 ? "L+NY" : "NY";
                return (
        ...
        {/* ── best / worst trade ── */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: gridGap }}>
          <TradeCard trade={best_trade}  label="Best Trade Ever" />
          <TradeCard trade={worst_trade} label="Worst Trade Ever" />
        </div>

        {/* ── breakdowns ── */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: gridGap }}>

          {/* symbol */}
          <div style={card()}>
            {head("Symbol Breakdown")}
          ...
          <div style={card()}>
            ...
            <div style={{ padding: "10px 12px 14px" }}>
              {style_breakdown.map(s => (
                <div key={s.style} style={{ marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, fontSize: 11 }}>
                    <span style={{ fontWeight: 900 }}>{s.style} <span style={{ color: S, fontWeight: 400 }}>({s.trades})</span></span>
                    <span style={{ color: clr(s.pnl) }}>{dollar(s.pnl)}</span>
                  </div>
                  <MiniBar pct={s.win_rate} color={s.win_rate >= 50 ? GN : RD} />
                  <div style={{ fontSize: 9, color: S, marginTop: 3 }}>Avg hold {s.avg_hold}</div>
                </div>
              ))}
            </div>
          </div>
